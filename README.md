...
If you are using this image in a K8 cluster, for Pod to Pod communcation, or using a reverse proxy like Traefik,
the environment variable `LAN_NETWORK` needs to be set to your CNI's subnet (typically a 10.* CIDR)
This subnet is created when first initializing the cluster.  
  ...
  template:
    ...
    spec:
      ...
      - name: arch-delugevpn
        ...
        - mountPath: /config/wireguard/wireguard.conf
          name: your-wireguard-volume-or-secret-configmap
      volumes:
       - name: your-data-volume
       ...
```
